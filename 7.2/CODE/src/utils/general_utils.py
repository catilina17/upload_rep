...
def check_version_templates(wb = None, path="", version="",  warning=False, open=False):
    err_mapp = "Vous n'avez pas la bonne version du fichier '%s'. Demandez à votre administrateur la version %s" % (path, version)
    if open:
        wb = ex.load_workbook_openpyxl(path, read_only=True)
    if wb.properties.description not in str(version).split(","):
        if not warning:
