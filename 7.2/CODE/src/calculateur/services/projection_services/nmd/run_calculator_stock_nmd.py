...
def run_calculator_nmd_stock(dar, horizon, nmd_source_files, name_product, cls_nmd_spreads=[], tx_params=[],
                             ...
                             batch_size = 10000, output_mode="dump", tci_contract_perimeter=[], exec_mode="simul"):
    try:

        cls_nmd = load_nmd_data(nmd_source_files["STOCK"], dar, nmd_source_files["MODELS"]["NMD"]["DATA"],
                                horizon, exec_mode)
...
def load_nmd_data(source_data, dar_usr, model_wb, horizon, exec_mode):
    ...
    cls_nmd = Data_NMD_ST_Formater(cls_data, cls_format, source_data, dar_usr, model_wb, horizon, exec_mode=exec_mode)
