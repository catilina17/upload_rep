...
from modules.moteur.services.projection_stock.scenario_dependent_layers import Stock_Updater as su
from mappings import general_mappings as gmp
from mappings import mapping_functions as mf
...
class RcoLikeReport():
    def __init__(self, cls_fields, sim_params, list_cls_agreg, horizon, output_folder, name_run):
        ...
        self.liquidity_report_indics = {"A$LEF": "GAP-LIQ-EF", "B$LEM": "GAP-LIQ-EM",
                                        ...
                                        pa.NC_PA_LMN_EVE: "MNI-EVE"}
        self.all_cols_num = ["M" + str(i) for i in range(0, horizon + 1)]
        self.cols_num_out = pa.NC_PA_COL_SORTIE_NUM_ST
        ...
        self.output_vars = [self.cls_fields.NC_LDP_BASSIN, self.cls_fields.NC_LDP_ETAB, self.NC_DARNUM,
                            ...
                            self.NC_OUTPUT_CUR, self.cls_fields.NC_LDP_RATE_CODE, self.cls_fields.NC_LDP_MARCHE,
                            self.NC_OUTPUT_GESTION,
    ...
    def generate_rco_like_reports(self):
        ...
        if len(compiled_data) > 0:
            ...
            compiled_data = self.filter_dim_and_market_ag_data(compiled_data)
            compiled_data = self.calculate_lmn_eve_indic(compiled_data)
    ...
    def calculate_lmn_eve_indic(self, compiled_data):
        compiled_data = su.calculate_lmn_gp_reg(compiled_data, "E$LMN", "G$LMN_GPTX", "LMN_GPRG",
                                                self.all_cols_num, gmp.mapping_gp_reg_params, self.cls_fields.NC_LDP_RATE_CODE)
        compiled_data = su.calculate_lmn_gp_reg(compiled_data, "F$LMN_FTP", "H$LMN_FTP_GPTX",
                                                "LMN_GPRG" + "_FTP", self.all_cols_num, gmp.mapping_gp_reg_params, self.cls_fields.NC_LDP_RATE_CODE)
        compiled_data = mf.map_data(compiled_data, gmp.map_pass_alm["CONTRATS"], keys_data=[ "BILAN_MAP", self.cls_fields.NC_LDP_CONTRACT_TYPE],
                            name_mapping="STOCK DATA vs.", cols_mapp = [pa.NC_PA_DIM2])
        compiled_data = su.calculate_lmn_eve(compiled_data, gmp.mapping_eve, self.cls_fields.NC_LDP_RATE_CODE,
                                             self.cls_fields.NC_LDP_ETAB, self.cls_fields.NC_LDP_CONTRACT_TYPE,
                                             pa.NC_PA_DIM2, "E$LMN", "F$LMN_FTP")


        return compiled_data

    def filter_dim_and_market_ag_data(self, data):
        if self.cls_fields.NC_LDP_DIM6 in data.columns:
            filter_rco = (data[self.cls_fields.NC_LDP_DIM6].isnull()) | (
                        data[self.cls_fields.NC_LDP_DIM6].astype(str).str.upper() != "FCT")
            filter_rco = filter_rco & (data[self.cls_fields.NC_LDP_MARCHE].str.upper() != "MDC")
    ...
    def format_report_data(self, compiled_data, indic):
        ...
        formated_data_indic, cols_unpivot = self.format_num_cols(compiled_data_indic)
        return formated_data_indic

    def format_num_cols(self, compiled_data_indic):
        compiled_data_indic = compiled_data_indic[self.output_vars + self.cols_num_out].copy()
        compiled_data_indic = compiled_data_indic.groupby(self.output_vars, as_index=False, dropna=False).sum()
        compiled_data_indic, cols_unpivot = self.rename_date_columns(compiled_data_indic, self.cols_num_out)
    ...
    def save_data_indic(self, formated_data_indic, name_indic):
        file_name = self.sim_params.etab.upper() + "_" + self.sim_params.dar.strftime(
            "%Y-%m-%d") + "_" + name_indic + ".tab"
    ...
    def add_missing_cols(self, data):
        ...
        values = ["ACTIF", "PASSIF", "ACTIF", "PASSIF", "PASSIF", "ACTIF"]
        data[self.NC_OUTPUT_BILAN] = np.select(cases, values, default="ACTIF")

        values = ["B ACTIF", "B PASSIF", "HB ACTIF", "HB PASSIF", "HB PASSIF", "HB ACTIF"]
        data["BILAN_MAP"] = np.select(cases, values, default="B ACTIF")
