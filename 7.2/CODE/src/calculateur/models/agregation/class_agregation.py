...
class Agregation():
    ...
    def store_compiled_indics(self, dic_inds, data_ldp, data_optional=[]):
        ...
        results_ag = (results_ag.groupby(self.ag_vars, dropna=False, as_index=False).sum(numeric_only=True))
        if self.output_mode in ["dump", "all"]:
            self.dump_in_temp_dir(results_ag)

        if self.output_mode in ["dataframe", "all"]:
            self.compiled_indics.append(results_ag.copy())
    ...
    def final_wrap(self):
        if len(self.compiled_indics) > 0:
            ...
            self.compiled_indics = data_compil.groupby(self.ag_vars, dropna=False, as_index=False).sum(
                numeric_only=True)
