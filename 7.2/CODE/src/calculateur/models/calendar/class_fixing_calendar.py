...
class Fixing_Calendar_Manager():
    ...
    def get_fixing_parameters(self, cls_proj):
        ...
        if self.name_product not in models_cap_floor:
    ...
    def get_periodic_calendar_per_type_of_product(self, data_ldp, col_fix_next_date, period_begin_date, current_month,
        ...
        if self.name_product not in (models_nmd_st + models_nmd_pn):
            ...
            if not self.name_product in (models_ech_pn + models_repos):
            ...
            if self.name_product in models_repos:
                decalage_deb_per = data_ldp[self.cls_fields.NC_DECALAGE_DEB_FIXING_PER].values.reshape(n, 1)
    ...
    def update_fixing_date(self, data_ldp, fixing_calendar, col_fix_next_date, col_fixing_nb_days,
        ...
        fixing_date = np.where(fixing_date > mat_date_np, mat_date_np, fixing_date)
