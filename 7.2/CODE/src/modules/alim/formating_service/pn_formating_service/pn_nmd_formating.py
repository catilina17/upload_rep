...
class PN_NMD():
    def __init__(self, cls_usr, cls_sp):
        ...
        self.cls_sp.batch_size_nmd = self.cls_sp.batch_size_nmd
    ...
    def generate_templates(self, data_pn_nmd, is_pn=True):
        ...
        rco_allocation_key = True if self.up.current_etab != "ONEY" else False

        cls_template = run_tmp.run_nmd_template_getter(source_data, self.up.current_etab, self.up.dar, format_data=False,
                                                       save=False, rco_allocation_key=rco_allocation_key)

        cls_template.data_template_mapped["BASSIN"] = self.up.current_etab

        if not is_pn or len(data_pn_nmd) ==0 :
    ...
    def get_nmd_source_data(self, data_pn_nmd):
        ...
        if self.up.current_etab != "ONEY":
            source_data["STOCK"] ["LDP"]["CHEMIN"] = self.up.nmd_st_files_name["ST-NMD"]
        ...
        source_data["MODELS"]["NMD"]["DATA"] = ex.load_workbook_openpyxl(self.up.modele_nmd_file_path, read_only=True)
    ...
    def get_oney_formated_stock(self, source_data_st):
        source_data_st["LDP"]["CHEMIN"] = self.up.main_files_name
        ...
        for col in data_st_oney_nmd.columns.tolist():
            if col[-5:]=="_CALC" and (data_st_oney_nmd[col].str.replace(" ","") == "[DAR]+1").any():
                data_st_oney_nmd[col[:-5]] = datetime.datetime.strftime(self.up.dar + relativedelta(days=1), "%d/%m/%Y")
    ...
    def get_calculator_params(self, data_pn_nmd, data_template_mapped):
        max_pn = pa.MAX_MONTHS_PN
        rcm = Calculator_RateParams_Manager(is_formated = False,
                                            scenario_name=self.up.sc_ref_nmd,
                                            rate_file_path= self.up.rate_file_path,
                                            liq_file_path=self.up.liq_file_path)
        tx_params = rcm.load_rate_params()
    ...
    def run_calage_simulation(self, data_pn_nmd, data_template_mapped):
        logger.info("   Démarrage de la simulation de calage avec le scénario %s" % self.up.sc_ref_nmd)

        nmd_source_files, tx_params, max_pn, etab, horizon, cls_temp\
            = self.get_calculator_params(data_pn_nmd, data_template_mapped)
        ...
        cls_ag_st\
            = nmd_st.run_calculator_nmd_stock(self.up.dar, horizon, nmd_source_files, "nmd_st",
                                              cls_nmd_spreads=cls_nmd_spreads, tx_params=tx_params,
                                              exit_indicators_type=["GPLIQ"], agregation_level="NMD_TEMPLATE",
                                              with_dyn_data=True, with_pn_data=True, batch_size=self.cls_sp.batch_size_nmd,
                                              output_mode="dataframe")

        logger.info("     Run de calage sur les PNs NMD")
        pn_to_generate \
            = nmd_pn.run_calculator_pn_nmd(self.up.dar, horizon, nmd_source_files, "nmd_pn", etab,
                                           ...
                                           type_rm="NORMAL", batch_size=self.cls_sp.batch_size_nmd, output_data_type="pn_monhtly_flow",
                                           output_mode = "dataframe")
        ...
        ex.close_workbook(nmd_source_files["MODELS"]["NMD"]["DATA"])

        return flux_pn

    def generate_stock_NMD_templates(self):
        logger.info("TRAITEMENT DES TEMPLATES NMD")
        cls_tmp = self.generate_templates([], is_pn=False)
