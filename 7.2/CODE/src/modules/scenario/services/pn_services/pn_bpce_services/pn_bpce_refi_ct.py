...
def generate_PN_REFI_CT(etab, file_path):
    wb = None
    try:
        if os.path.exists(file_path):
            ...
            wb = ex.load_workbook_openpyxl(file_path, read_only=True)
            ...
            if data_refi_ct.shape[0] != 0:
                ...
                data_refi_ct = mp.map_data(data_refi_ct, data_map, keys_data=["INDEXO"], option="", error_mapping=False)
                data_refi_ct[pa.NC_PA_LCR_TIERS].mask(data_refi_ct["type_lcr"] == "lcr60",
                                                        data_refi_ct[rf_bpce.NC_LCR_TIERS_60],
                                                        inplace=True)
                data_refi_ct = mp.map_data(data_refi_ct, mapping_conso_ech, keys_data=[pa.NC_PA_CONTRACT_TYPE],
                                             join_how="inner")
                data_refi_ct = data_refi_ct.drop([pa.NC_PA_MATUR], axis=1)
                data_refi_ct = mp.map_data(data_refi_ct, mapping_refi_bpce, keys_data=[pa.NC_PA_CONTRACT_TYPE],
                                             join_how="inner")
