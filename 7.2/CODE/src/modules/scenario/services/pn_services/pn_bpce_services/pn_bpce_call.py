...
def generate_PN_call(DAR, etab, file_path):
    ...
    if os.path.exists(file_path):
        ...
        mapping_BPCE = mp.mapping_bpce_pn
        mapping_refi_bpce = mp.mapping_bpce_pn["REFI BPCE"]
        ...
        call_data["MONTH_START"] = 1 + call_data[rf_bpce.NC_CALL_DATE].dt.year * 12 + call_data[
            rf_bpce.NC_CALL_DATE].dt.month - (12 * DAR.year + DAR.month)
        ...
        if len(call_data) > 0:
            ...
            filtero_mois = filtero_mois & np.array((call_data_txsp[pa.NC_PA_RATE_CODE] == "FIXED")).reshape(
