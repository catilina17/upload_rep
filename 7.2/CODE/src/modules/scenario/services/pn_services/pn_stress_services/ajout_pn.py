...
def ajout_pn_scenario(pn_output_path, pn_type, scenario_stress_pn, etab):
    pn_data = scenario_stress_pn.copy()
    pn_data = pn_data[pn_data[pa.NC_PA_BASSIN] == etab].copy()
    ...
    pn_file_path = pr._get_file_path(pn_output_path, file_substring='NMD' if 'NMD' in pn_type else "PN_ECH",
                                     no_files_substring=["NMD_BC", "NMD_CALAGE", "PN_ECH_BC"])
    old_pn_df = pd.read_csv(pn_file_path, sep=";", decimal=",")

    if pn_data.shape[0] > 0:
        pn_data = add_missing_key_cols(pn_data, etab)
        ...
        if "NMD" in pn_type:
            pn_data_ajout = verify_uniqueness(pn_data_ajout, old_pn_df, etab)

        n = old_pn_df.shape[0]

        filter_replace = old_pn_df.index.isin(pn_data_remplacement.index.values.tolist()).reshape(n, 1)
        old_df_replace = old_pn_df[filter_replace].copy()
        old_df_no_replace = old_pn_df[~filter_replace].copy()

        old_df_replace[pa.NC_PA_COL_SORTIE_NUM_PN] = np.nan

        old_df_replace.update(pn_data_remplacement[pa.NC_PA_COL_SORTIE_NUM_PN].astype(str).replace("", "nan").astype(np.float64))

        new_pn_data = pd.concat([old_df_no_replace.reset_index(), old_pn_df.reset_index(), pn_data_ajout], ignore_index=False)

        new_pn_data = final_format(new_pn_data, pn_type)

        new_pn_data.to_csv(pn_file_path, sep =";", decimal = ",", index=False)
...
def map_pn(pn_data):
    ...
    pn_data = mp.mapping_consolidation_liquidite(pn_data)

    pn_data = mp.map_data(pn_data, mapp.map_pass_alm["CONTRATS2"], \
                            keys_data=[pa.NC_PA_CONTRACT_TYPE], name_mapping="PN DATA vs.")

    pn_data = mp.map_data(pn_data, mapp.map_pass_alm["INDEX_AGREG"], \
