...
class GAP_TAUX_PN_ECH():
    def __init__(self, cls_usr, cls_mp):
        self.up = cls_usr
        self.mp = cls_mp

    def calculate_mat_gap_tf_ech(self, main_data, max_month_pn, ht, lg, dic_mtx_pn_tx_fix_f, dic_mtx_pn_tx_fix_m,
                                 mtx_pn_f,
                                 mtx_pn_m, mtx_tef, mtx_tem):
        """ FILTRES POUR CHAQUE CAS"""
        s = (ht, max_month_pn, lg)
        fix_cases = self.generate_fixing_cases(main_data)
        tab_fix = self.generate_fixing_tables(max_month_pn, lg, fix_cases)
        fix_cases = gf.generate_list_from_ordered_dic(fix_cases)
        for k in range(1, self.up.nb_mois_proj_out + 1):
            ...
            if do_f:
                dic_mtx_pn_tx_fix_f[k] = self.calc_fix_mtx(main_data, mtx_pn_f, mtx_tef, fix_cases, tab, s, k)

            if do_m:
                dic_mtx_pn_tx_fix_m[k] = self.calc_fix_mtx(main_data, mtx_pn_m, mtx_tem, fix_cases, tab, s, k)

    def calc_fix_mtx(self, main_data, mtx, mtx_gp, fix_cases, tab, s, k):
    ...
    def generate_fixing_tables(self, max_month_pn, lg, cases_fix):
        tab_fix = {}
        for k in range(1, self.up.nb_mois_proj_out + 1):
            ...
            if do_f or do_m:
                ...
                self.calc_tab_fix_k_ech(tab_fix, k, cases_fix, max_month_pn, lg, n)

        return tab_fix

    def calc_tab_fix_k_ech(self, tab_fix, k, cases_fix, max_month_pn, lg, n):
    ...
    def generate_fixing_cases(self, main_data):
        per_fx = self.generate_freq_fixing_periodicity(main_data, gp.nc_pn_periode_fixing, gp.nc_pn_periode_interets)
        per_fx = np.where(main_data[gp.nc_pn_periode_fixing] == "1D", 0, per_fx)
        per_fx = np.where(main_data[gp.nc_output_index_calc_cle].str.contains(gp.FIX_ind), self.up.nb_mois_proj_usr + 5,
                          per_fx)
    ...
    def generate_freq_fixing_periodicity(self, data, col_fixing_periodicity, col_periodicity):
        periodicity = np.select(
            [data[col_periodicity] == x for x in ["MENSUEL", "TRIMESTRIEL", "NONE", "ANNUEL", "SEMESTRIEL"]],
