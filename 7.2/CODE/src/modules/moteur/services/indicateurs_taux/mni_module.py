...
class MNI_Calculator():
    def __init__(self, cls_usr, cls_mp, cls_tx):
        self.up = cls_usr
        self.mp = cls_mp
        self.tx = cls_tx

    def calculate_mni_st(self, dic_stock_sci, indic_sortie):
    ...
    def calculate_mni_pn(self, dic_pn_sci, data_dic, simul, type_pn, cols):
        """ calcul de la mni tx, mg, pour la PN """
        mtx_tx_pur = None;
        mtx_tx_mg = None

        if gf.begin_with_list(self.up.indic_sortie["PN"], gp.mn_pni) or gf.begin_with_list(self.up.indic_sortie["PN"],
                                                                                      gp.mn_tx_pni):
        ...
        """ Indicateurs simples"""
        if (gp.mn_tx_pni in self.up.indic_sortie["PN"] or gp.mn_pni in self.up.indic_sortie["PN"]) and simul == "LIQ":
            dic_pn_sci[gp.mn_tx_pni] = gf.sum_each2(mtx_tx_pur, cols)

        if (gp.mn_mg_pni in self.up.indic_sortie["PN"] or gp.mn_pni in self.up.indic_sortie["PN"]) and simul == "LIQ":
            dic_pn_sci[gp.mn_mg_pni] = gf.sum_each2(mtx_tx_mg, cols)

        if gp.mn_pni in self.up.indic_sortie["PN"] and simul == "LIQ":
        ...
        for p in range(1, self.up.nb_mois_proj_out + 1):
            if gp.mn_tx_pni + str(p) in self.up.indic_sortie["PN"] and simul == "LIQ":
                dic_pn_sci[gp.mn_tx_pni + str(p)] = gf.sum_each2(mtx_tx_pur, cols, proj=True, per=p,
                                                                 interv=self.up.indic_sortie["PN_CONTRIB"][gp.mn_tx_pni])

            if gp.mn_mg_pni + str(p) in self.up.indic_sortie["PN"] and simul == "LIQ":
                dic_pn_sci[gp.mn_mg_pni + str(p)] = gf.sum_each2(mtx_tx_mg, cols, proj=True, per=p)

            if gp.mn_pni + str(p) in self.up.indic_sortie["PN"] and simul == "LIQ":
                dic_pn_sci[gp.mn_pni + str(p)] = gf.sum_each2(mtx_tx_pur + mtx_tx_mg, cols, proj=True, per=p, \
                                                              interv=self.up.indic_sortie["PN_CONTRIB"][gp.mn_pni])
            ...
            if di.mni_gpliq_pn[p] and simul == "EVE_LIQ":
                dic_pn_sci[gp.mn_gpliq_pni + str(p)] = dic_pn_sci[gp.mn_gpliq_tx_pni + str(p)] + dic_pn_sci[
                    gp.mn_gpliq_mg_pni + str(p)]
    ...
    def force_marge_co_to_zero_for_tci_contracts(self, data_pn, tx_mg_sc):
        ht = data_pn.shape[0]
        filtre_contrat_tci = np.array(data_pn[gp.nc_output_dim2].isin(self.mp.mapping_eve["cc_tci"])).reshape(ht, 1, 1)
        filtre_contrat_tci_excl = ~np.array(data_pn[gp.nc_output_contrat_cle].isin(self.mp.mapping_eve["cc_tci_excl"])).reshape(ht, 1, 1)
        new_tx_mg_sc = np.where(filtre_contrat_tci & filtre_contrat_tci_excl, 0, tx_mg_sc)
        return new_tx_mg_sc

    def force_mni_to_mni_tci_for_nmd_tci_contracts(self, data_pn, tx_sc, tx_sc_ftp):
        ht = data_pn.shape[0]
        filtre_contrat_tci = np.array(data_pn[gp.nc_output_dim2].isin(self.mp.mapping_eve["cc_tci"])).reshape(ht, 1, 1)
        filtre_contrat_tci_excl = ~np.array(data_pn[gp.nc_output_contrat_cle].isin(self.mp.mapping_eve["cc_tci_excl"])).reshape(ht, 1, 1)
    ...
    def calculate_mni_mtx_gp_rg_pn(self, dic_pn_sci, dic_mtx_gp, data_dic, cols, type_pn):
        ...
        for p in range(1, self.up.nb_mois_proj_out + 1):
            if di.mni_gpr_tx_pn[p]:
                if "ech" in type_pn:
                    ...
                    new_tx_sc, filter_tci = self.force_mni_to_mni_tci_for_nmd_tci_contracts(data_dic["data"],
                                                                                       data_dic["tx_sc_"],
                                                                                       data_dic["tx_sc_ftp_"])
            ...
            if di.mni_gpr_mg_pn[p]:
                if "ech" in type_pn:
                    tx_mg = self.force_marge_co_to_zero_for_tci_contracts(data_dic["data"], data_dic["tx_mg_"])
                else:
    ...
    def calculate_mni_gp_rg_nmd(self, dic_pn_sci, data_dic):
        ...
        for p in range(1, self.up.nb_mois_proj_out + 1):
            if di.mni_gpr_tx_pn[p]:
                new_tx_sc, filter_tci = self.force_mni_to_mni_tci_for_nmd_tci_contracts(data_dic["data"], data_dic["tx_sc_"],
                                                                                   data_dic["tx_sc_ftp_"])
                new_tx_sc = np.where(filter_tci.reshape(ht, 1),
                                     (data_dic["tx_sc_"][:, :p] * data_dic["mtx_em"][:, :p]).sum(axis=1)
    ...
    def calculate_mni_ajust_gp_rg(self, dic_ajust, num_cols):
        ...
        if (sum(list(di.mni_gpr_tx_aj.values())) + sum(list(di.mni_gpr_tx_aj.values()))
            ...
            tx_sc = (data_adj_tx[["CURVE_NAME", "TENOR"]].copy()
                     .join(tx_curve.set_index(["CODE COURBE", "MATURITE"]), how="left", on=["CURVE_NAME", "TENOR"]))

            tx_sc["M0"] = 0
            tx_sc = tx_sc[["M" + str(i) for i in range(0, self.up.nb_mois_proj_usr + 1)]]
            tx_sc = np.array(tx_sc)

            """ chargement des TX LIQ """
            sp_liq = \
                data_adj_tx[[gp.nc_output_devise_cle]].copy().merge(self.up.spread_liq, how="left",
                                                                    right_on=gp.nc_devise_spread,
    ...
    def calculate_mni_ajust(self, dic_ajust, num_cols):
        ...
        """ chargement des TX LIQ """
        default = ~data_adj_tx[gp.nc_output_devise_cle].isin(self.up.spread_liq[gp.nc_devise_spread].values.tolist())
        data_adj_tx.loc[default, gp.nc_output_devise_cle] = "*"
        sp_liq = \
            data_adj_tx[[gp.nc_output_devise_cle]].copy().merge(self.up.spread_liq, how="left", right_on=gp.nc_devise_spread,
                                                                left_on=gp.nc_output_devise_cle)[gp.nc_taux_spread]
        """ MNI TX"""
        ...
        """ TX CLI"""
        if gp.tx_cli_pni in self.up.indic_sortie["AJUST"]:
        ...
        for p in range(1, self.up.nb_mois_proj_out + 1):
            ...
            for ind_mni in [gp.mn_tx_pni, gp.mn_mg_pni, gp.mn_pni]:
                if ind_mni + str(p) in self.up.indic_sortie["AJUST"]:
