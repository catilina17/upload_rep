...
class GAP_TAUX_PN_NMD():
    def __init__(self, cls_usr, cls_mp):
        self.up = cls_usr
        self.mp = cls_mp
    
    def calculate_mat_gap_tf_nmd(self, main_data, dic_stock_scr, simul, ht, lg, type_tx_pn, filtre_tla, max_month_pn, mtx_ef, mtx_em,
        ...
        tab_fix_f, tab_fix_m = self.load_coeff_fix_st(main_data, dic_stock_scr, ht, lg)
        ...
        for k in range(1, self.up.nb_mois_proj_out + 1):
            ...
            f_mois_refix = (self.up.mois_refix_tla >= k + 1)
            ...
            if do_f:
                mtx_fix_f = self.refix_encours_mtx(mtx_ef, tab_fix_f, n, k)
    
            if do_m:
                mtx_fix_m = self.refix_encours_mtx(mtx_em, tab_fix_m, n, k)
    
            if do_m or do_f:
                if not prct == "%":
                    tab_fix_tla_roll = self.calc_tab_refix_tla(f_mois_refix, ht, lg, k)
                    if do_f:
                        mtx_fix_f = self.refix_tla_nmd(mtx_ef, mtx_fix_f, tab_fix_tla_roll, k, n, filtre_tla_all, filtre_tf_all)
                    if do_m:
                        mtx_fix_m = self.refix_tla_nmd(mtx_em, mtx_fix_m, tab_fix_tla_roll, k, n, filtre_tla_all, filtre_tf_all)
            if do_f:
    ...
    def calc_tab_refix_tla(self, f_mois_refix, ht, lg, k):
        tab_fix = np.zeros((ht, lg))
        if f_mois_refix:
            tab_fix[:, :self.up.mois_refix_tla] = 1
        else:
            tab_fix[:, :self.up.freq_refix_tla] = 1
    ...
    def refix_tla_nmd(self, mtx_encours, mtx_fix_f, tab_fix_tla_roll, k, n, filtre_tla_all, filtre_tf_all):
    ...
    def refix_encours_mtx(self, mtx_encours, tab_fix_f, n, k):
    ...
    def load_coeff_fix_st(self, main_data, dic_stock_scr, ht, lg):
