...
def refix_tla_stock(mp, up, data_stock, is_ech_stock, dic_stock_sci):
    """ FILTRE TLA, TLB, LEP, CEL """
    filtre_tla = (np.array(is_ech_stock) == False) & (
        np.array(data_stock[gp.nc_output_index_calc_cle].isin(mp.mapping_gp_reg_params["all_gap_gestion_index"] ))) & np.array([up.retraitement_tla])
