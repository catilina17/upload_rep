...
def project_nmd(pnc, mp, up, cls_calc, cls_stock_calc, cls_pn_loader,
    ...
    if "nmd" in cls_pn_loader.dic_pn_nmd:
        ...
        dic_pn_sc = {}
        cls_nmd = PN_NMD_PROJECTER(mp, up, cls_calc, cls_stock_calc, cls_pn_loader, scen_path, name_scenario, "")
        cls_nmd.project_pn_nmd(dic_pn_sc, dic_stock_scr)
        pnc.compile_pn(dic_pn_sc, "nmd", name_scenario, new_dir, dic_stock_scr=dic_stock_scr)


def project_nmd_prct(pnc, mp, up, cls_calc, cls_stock_calc, cls_pn_loader,
                     dic_stock_scr, name_scenario, scen_path, new_dir):
    if "nmd%" in cls_pn_loader.dic_pn_nmd:
        logger.info("       PROJECTION ET COMPILATION DES PN NMD%")
        dic_pn_sc = {}
...
def project_ech(cal, pnc, mp, up, cls_calc, cls_pn_loader, name_scenario, scen_path, new_dir):
    if "ech" in cls_pn_loader.dic_pn_ech:
        ...
        for i in range(0, len(data_ech)):
            dic_pn_sc = {}
            cls_ech = PN_ECH_PROJECTER(cal, mp, up, cls_calc, cls_pn_loader, scen_path, name_scenario)
            cls_ech.project_pn_ech(data_ech[i].copy(), dic_pn_sc)
            pnc.compile_pn(dic_pn_sc, "ech", name_scenario, new_dir)


def project_ech_prct(cal, pnc, mp, up, cls_calc, cls_pn_loader, name_scenario, scen_path, new_dir, dic_stock_sci):
    if "ech%" in cls_pn_loader.dic_pn_ech:
        logger.info("       PROJECTION ET COMPILATION DES PN ECH%")
        ...
        for i in range(0, len(data_ech_prct)):
            dic_pn_sc = {}
            cls_ech = PN_ECH_PROJECTER(cal, mp, up, cls_calc, cls_pn_loader, scen_path, name_scenario, ech_prct="%")
            cls_ech.project_pn_ech(data_ech_prct[i].copy(), dic_pn_sc, dic_stock_sci=dic_stock_sci)
            pnc.compile_pn(dic_pn_sc, "ech%", name_scenario, new_dir)


def calculate_ajustements(am, up, name_scenario, new_dir):
