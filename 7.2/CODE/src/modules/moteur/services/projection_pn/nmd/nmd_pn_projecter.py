...
from modules.moteur.services.indicateurs_taux.nmd.nmd_mtx_gap_tf import GAP_TAUX_PN_NMD
...
class PN_NMD_PROJECTER():
    ...
    def __init__(self, cls_mp, cls_usr, cls_calc, cls_stock_calc, cls_pn_loader, scen_path, scenario_name, nmd_prct):
    ...
    def project_pn_nmd(self, dic_pn_sc, dic_stock_scr):
        ...
        (self.up.indic_sortie["PN"] + self.up.indic_sortie_eve["PN"] == []
         and self.up.indic_sortie["AJUST"] + self.up.indic_sortie_eve[
    ...
    def get_contrats_files(self, etab, nom_produit):
        sources_file = gmp.nomenclature_contrats[gmp.nomenclature_contrats["TYPE CONTRAT"] == nom_produit].copy()
        sources_file = sources_file[sources_file["ENTITE"] == "RZO"].copy()
        sources_file["CHEMIN"] = sources_file["CHEMIN"].str.replace("RZO", etab)
        sources_file["CHEMIN"] = [os.path.join(self.up.source_path, *x.split('\\')) for x in sources_file["CHEMIN"]]
    ...
    def get_ecoulements_from_calculateur(self, pn_nmd_source_data):
        ...
        nmd_source_data["STOCK"] = self.sources_cls.get_contract_sources_paths(self.params.etab, self.params.products_map_nomenclature["nmd_st"])
        ...
        dic_sc_vars \
            = pn_nmd.run_calculator_pn_nmd(self.params.dar, self.params.horizon,
                                           ...
                                           batch_size=self.params.batch_size_nmd, tci_contract_perimeter=self.params.nmd_tci_perimeter,
                                           output_data_type="pn_proj", output_mode = "dataframe")

        ex.close_workbook(nmd_source_data["MODELS"]["NMD"]["DATA"])
    ...
    def cal_mtx_gp_tf(self, data_pn, dic_pn_sc, mtx_ef, mtx_em, dic_stock_scr, ht, lg):
        ...
        filtre_tla = (np.array(data_pn[gp.nc_output_index_calc_cle].isin(self.mp.all_gap_gestion_index))) & np.array(
            [self.up.retraitement_tla])

        gp_nmd = GAP_TAUX_PN_NMD(self.up, self.mp)
    ...
    def calc_vol_pn(self, dic_pn_sc, dic_sc_vars):
        ...
        if self.up.nb_mois_proj_usr - y > 0:
            dic_pn_sc["new_pn"] = np.concatenate([dic_pn_sc["new_pn"], np.zeros((x, self.up.nb_mois_proj_usr - y))], axis=1)
