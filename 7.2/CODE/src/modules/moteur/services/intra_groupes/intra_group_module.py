...
class IG_Manager():
    def __init__(self, cls_usr, cls_mp):
        self.up = cls_usr
        self.mp = cls_mp
        self.mp.adapt_map_to_sef()
        self.WARNING_BASSIN = "        Les données de PN %s semblent appartenir à un bassin différent de celui spécifié dans les paramètres"

    def add_ig_colum(self, data, palier_col, contrat_col):
        """ Fonction permettant d'ajouter la colonne isIG"""
        if not self.up.bassin_usr in data[gp.nc_output_bassin_cle].values.tolist() and data.shape[0] != 0:
            logger.warning(self.WARNING_BASSIN)

        data["BASSIN_IG"] = self.mp.bassin_ig_map.loc[self.up.bassin_usr, gp.nc_igm_bassinig]

        data["CONTREPARTIE"] = data.join(self.mp.bassin_ig_map[[gp.nc_igm_bassinig]], on=palier_col)[
            gp.nc_igm_bassinig].copy()

        data["CLE_IG"] = data[contrat_col] + "_" + data["BASSIN_IG"] + "_" + data["CONTREPARTIE"]

        filtre_ig = data["CLE_IG"].isin(self.mp.ig_mirror_contracts_map.index.values.tolist())

        filtre_ig = (filtre_ig) & (
                    (data[palier_col] != self.up.bassin_usr) | (data[contrat_col].astype(str).str[:1] != "P")) & \
                    (~  ((data[palier_col] != self.up.bassin_usr) & (data["BASSIN_IG"] == "RZO") & \
                         ((data[palier_col] == "CFF") | (
    ...
    def add_mirror_contract(self, data):
    ...
    def add_counterparty_contract(self, data):
        ...
        if data_c.shape[0] > 0:
            ...
            data_c["BASSIN_IG"] = self.mp.bassin_ig_map.loc[self.up.bassin_usr, gp.nc_igm_bassinig]

            data_c["CONTREPARTIE"] = \
            temp_palier.join(self.mp.bassin_ig_map[[gp.nc_igm_bassinig]], on=gp.nc_output_palier_cle)[
                gp.nc_igm_bassinig].copy()

            data_c["CLE_IG"] = data_c[gp.nc_output_contrat_cle] + "_" + data_c["BASSIN_IG"] + "_" + data_c[
                "CONTREPARTIE"]

            data_c[gp.nc_output_contrat_cle] = \
                data_c.join(self.mp.ig_mirror_contracts_map, how="left", on="CLE_IG", rsuffix="_IG")[
                    gp.nc_ig_contrat_new]

            for col in [gp.nc_output_bilan, gp.nc_output_poste, gp.nc_output_dim2, gp.nc_output_dim3, gp.nc_output_dim4,
                        gp.nc_output_dim5]:
                data_c[col] = \
                data_c.join(self.mp.contrats_map, how="left", on=gp.nc_output_contrat_cle, rsuffix="_MAP")[
                    col + "_MAP"]
            ...
            data_c = self.mp.map_liq(data_c)

            filtre_surcentralisation = np.where(np.array(temp_bilan) != np.array(data_c[gp.nc_output_bilan]), True,
                                                False)
            num_cols = ["M0" + str(k) if k < 10 else "M" + str(k) for k in range(0, self.up.nb_mois_proj_usr + 1)]
