...
def map_data(data_to_map, mapping, keys_data=[], keys_mapping=[], cols_mapp=[], override=False, name_mapping="", \
    ...
    if override:
        ...
        if new_data.shape[0] != data_to_map.shape[0]:
            raise ValueError("Impossible to override, indicators " + name_mapping + " is not unique")
...
def map_data_with_ntx_mappings(data, sc_ref_nmd, rate_file_path, liq_file_path):
    perim_NTX = ntx_p.perim_ntx
    if not perim_NTX in gma.mapping_NTX["MNI"]["TABLE"].columns:
        raise ValueError(
            "Le périmètre souhaité %s n'est pas dans le indicators %s" % (perim_NTX, gma.mapping_NTX["MNI"]["FULL_NAME"]))
    ...
    sc_curves_df = RatesManager.get_sc_df(sc_ref_nmd, rate_file_path, liq_file_path)
...
def map_data_with_general_mappings(data, current_etab):
    ...
    for i in range(1, len(mappings) + 1):
        if (not (current_etab in gp.MTY_DETAILED_ETAB and i == 3)) \
                and (not (current_etab not in gp.MTY_DETAILED_ETAB and i == 2)):
...
def map_intra_groupes(current_etab, data):
    ...
    if current_etab == "BPCE":
        check = check & (data["BASSIN IG"] == "BPCE") & (data["isBPCEIG"] == "")
    elif current_etab in gp.NTX_FORMAT:
    ...
    if sum(check) > 0:
        logger.warning(
            "PROBLEME SUR LES indicators DES CONTREPARTIES INTRAGROUPES: Veuillez vérifier les indicators manquants")
