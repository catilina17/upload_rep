...
def read_input_tx_rate_curve_from_wb(tx_file_path, scenario_name, range_name):
    input_taux_wb = ex.try_close_open(tx_file_path, read_only=True)
    rate_df = ex.get_dataframe_from_range(input_taux_wb, range_name)
...
def get_bonds_curve_with(rate_curve, bonds_spread):
    logger.debug('Création des courbes d\'obligation')
    num_cols = [x for x in range(0, gp.NB_PROJ_TAUX + 1)]
    num_cols_SWAP = [str(x) + "_SWAP" for x in range(0, gp.NB_PROJ_TAUX + 1)]
    bonds_curves = bonds_spread.merge(rate_curve, left_on=[gp.CN_CODE_PASSALM], right_index=True, how='inner',
                                       suffixes=(None, "_SWAP"))
    bonds_curves[num_cols] = bonds_curves[num_cols].values + bonds_curves[num_cols_SWAP].values
    bonds_curves = bonds_curves[[gp.CN_CODE_PASSALM] + num_cols].copy()
    bonds_curves.set_index(gp.CN_CODE_PASSALM, inplace=True)
    rate_curve.update(bonds_curves)
    return rate_curve