...
from . import maturities_referential as mr
import traceback
import logging
...
def construct_calc_curves(all_curves, pn_pricing_curves_ref, mapping_rate_code_curve):
        ...
        try:
            ...
            if True:#curve["CODE COURBE"] in ['LGBPASK']:
                ...
                if len(existing_maturities_diff_1) > 0:
                    ...
                    if (str(curve["RESULT TENOR"]) != "" and curve["RESULT TENOR"] is not None) and set(mat_l) != {"ALL"}:
    ...
    if len([x for x in list(set(list_curve_absent_mat)) if x not in gb_list_curve_absent_mat]) > 0:
        logger.warning("      Impossible de calculer les courbes suivantes car certaines de leurs courbes sous-jacentes sont absentes:")
