...
def generate_PN_REFI_SC(xl, NG_SC_PN_AUTRES_BASSINS, etab, scenario_name):
    ...
    if not scen_path is None and scen_path != "":
        ...
        for subdir, dir, files in sc_dir:
            for file_name in files:
                if ".xlsb" in file_name and "SC_VOLUME" in file_name and not file_name[0:2] == "~$":
                    if "_".join(scenario_name.split("_")[:-2]) in file_name:
                        logger.info("          Lecture de : " + file_name + " pour " + subdir.split("\\")[-1])

                        data_temp = ex.open_wb_and_get_data_from_named_range(xl, os.path.join(subdir, file_name),
                                                                             rf_bpce.NG_DATA_ECH_SC)
        ...
        if len(data_ech_bassins) > 0:
            ...
            data_ech_bassins = mapp.map_data(data_ech_bassins, mapping_refi, cols_mapp=cols_to_mapp,
