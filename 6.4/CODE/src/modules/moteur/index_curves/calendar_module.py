...
def load_calendar_coeffs():
    ...
    calendar_coeff["30E/360"] = [apply_30E_360_calc(x, y) for x, y in zip(calendar_coeff.Dcold, calendar_coeff.Dcolf)]

    calendar_coeff["30/360"] = [apply_30_360_calc(x, y) for x, y in zip(calendar_coeff.Dcold, calendar_coeff.Dcolf)]

    calendar_coeff = calendar_coeff.transpose().reset_index()

    calendar_coeff.columns = [gp.nc_pn_base_calc] + ["M" + str(i) for i in range(1, period + 1)]


def generate_base_calc_coeff_stock(data_stock, type_tx_stock):
    """ JOINT LES COEFFS BASE CALC POUR CHAQUE CONTRAT STOCK"""

    """ SELECTIONNE LES COLONNES POUR LE NB DE MOIS DE PROJ """
    coeff_calendar = calendar_coeff.copy()[
        [gp.nc_pn_base_calc] + ["M" + str(i) for i in range(1, up.nb_mois_proj_usr + 1)]]

    """ CONCATENATION DES PARAM BASE CALC NMD ET ECH"""
    cols = [gp.nc_nmd_basec_tf, gp.nc_nmd_basec_tv]
    base_calc_col = mp.param_tx_ech[cols].combine_first(mp.param_nmd_base_calc[cols])
    ...
    list_missing = base_calc_col.reset_index()[base_calc_col.reset_index()[gp.nc_pn_base_calc].isna()][gp.nc_output_contrat_cle].values.tolist()
    if list_missing!=[]:
        logger.warning("    The following base calc are missing for the following contracts : %s" % list(set(list_missing)))
