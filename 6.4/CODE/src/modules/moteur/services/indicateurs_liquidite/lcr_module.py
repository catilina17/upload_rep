...
global nco_dar_outflow_t0_tot, nco_dar_inflow_t0_tot, rl_dar_t0_tot
global erreur_outflow_t0, erreur_inflow_t0, em_total_pass_t0, em_total_act_t0, erreur_rl_t0
global nco_moteur_outflow_t0_tot, nco_moteur_inflow_t0_tot
global coeffs_lcr, coeffs_lcr_spec, bc_outflow_lcr

def load_lcr_params(wb):
    global coeffs_lcr, coeffs_lcr_spec, bc_outflow_lcr
    ...
    """ LCR DAR """
    if gp.rl_pni in up.indic_sortie["PN"] or gp.nco_pni in up.indic_sortie["PN"] \
            or gp.rl_sti in up.indic_sortie["ST"] or gp.rl_sti in up.indic_sortie["ST"]:
        read_input_lcr_dar(wb)
...
def read_input_lcr_dar(wb):
    ...
    if lcr_dar.shape[0] == 0:
        ...
        nco_dar_outflow_t0_tot = 0
        nco_dar_inflow_t0_tot = 0
        rl_dar_t0_tot = 0
    else:
        lcr_dar = lcr_dar[[gp.nc_indic_lcr, "VAL"]].copy()
        nco_dar_outflow_t0_tot = lcr_dar[lcr_dar[gp.nc_indic_lcr] == gp.nr_nco_outflow_lcr_dar]["VAL"].iloc[0] * 10 ** 9
        nco_dar_inflow_t0_tot = lcr_dar[lcr_dar[gp.nc_indic_lcr] == gp.nr_nco_inflow_lcr_dar]["VAL"].iloc[0] * 10 ** 9
        rl_dar_t0_tot = lcr_dar[lcr_dar[gp.nc_indic_lcr] == gp.nr_rl_lcr_dar]["VAL"].iloc[0] * 10 ** 9


def calc_err_em_tot(data, dic_indic, other_data):
    global nco_dar_outflow_t0_tot, nco_dar_inflow_t0_tot, rl_dar_t0_tot
    global erreur_outflow_t0, erreur_inflow_t0, em_total_pass_t0, em_total_act_t0, erreur_rl_t0
    ...
    if nco_indic in indic_sortie or rl_indic in indic_sortie:
        ...
        nco_coeff, rl_coeff = load_nco_rl_coeff(data, other_data, ht)
        ...
        em_total_pass_t0 = (em0 * not_hb * coeff_pass).sum()
        em_total_act_t0 = (em0 * not_hb * coeff_act).sum()

    if nco_indic in up.indic_sortie["ST"]:
        filter_outflow = other_data[gp.nc_output_bc].isin(bc_outflow_lcr)
        ...
        nco_outflow_t0 = np.where(filter_outflow.reshape(ht, 1), nco_outflow_t0_real, nco_outflow_t0)
        nco_outflow_t0_tot = (nco_outflow_t0).sum()

        nco_inflow_t0 = - coeff_bilan * em0 * nco_coeff * coeff_act * not_hb
        nco_inflow_t0_tot = (nco_inflow_t0).sum()

        erreur_outflow_t0 = - nco_dar_outflow_t0_tot - nco_outflow_t0_tot
        erreur_inflow_t0 = nco_dar_inflow_t0_tot - nco_inflow_t0_tot

    if rl_indic in indic_sortie:
        rl_moteur_t0 = -coeff_bilan * em0 * rl_coeff * not_hb
        rl_moteur_t0_tot = (rl_moteur_t0).sum()
        erreur_rl_t0 = rl_dar_t0_tot - rl_moteur_t0_tot


def calculate_nco_and_rl(dic_indic, coeff_bilan, indic_sortie, em_indic, nco_indic, rl_indic, delta_nco_indic, \
                         delta_rl_indic, hors_bilan, outfl_indic, nco_coeff, rl_coeff, filter_outflow):

    global nco_dar_outflow_t0_tot, nco_dar_inflow_t0_tot, rl_dar_t0_tot
    global erreur_outflow_t0, erreur_inflow_t0, em_total_pass_t0, em_total_act_t0, erreur_rl_t0
    global nco_moteur_outflow_t0_tot, nco_moteur_inflow_t0_tot, rl_moteur_t0_tot
    ...
    if nco_indic in indic_sortie:
        nco_moteur_outflow_t0 = - coeff_bilan * em0 * nco_coeff * coeff_pass * not_hb
        nco_moteur_outflow_t0_empreinte = - np.array(dic_indic[outfl_indic + " 0M-1M"]["M0"]).reshape(ht, 1)
        nco_moteur_outflow_t0 = np.where(filter_outflow.reshape(ht, 1), nco_moteur_outflow_t0_empreinte,
                                         nco_moteur_outflow_t0)
        nco_moteur_inflow_t0 = - coeff_bilan * em0 * nco_coeff * coeff_act * not_hb
        nco_t0 = (nco_moteur_outflow_t0 + erreur_outflow_t0 * em0 / em_total_pass_t0) * coeff_pass * not_hb
        nco_t0 = (nco_t0 + (nco_moteur_inflow_t0 + erreur_inflow_t0 * em0 / em_total_act_t0) * coeff_act) * not_hb
        dic_indic[nco_indic] = np.array(nco_t0).reshape(ht, 1) + dic_indic[delta_nco_indic]

    if rl_indic in indic_sortie:
        rl_moteur_t0 = -coeff_bilan * em0 * rl_coeff * not_hb
        rl_t0 = rl_moteur_t0 + (erreur_rl_t0 * em0 / em_total_act_t0 * coeff_act * not_hb)
        dic_indic[rl_indic] = np.array(rl_t0).reshape(ht, 1) + dic_indic[delta_rl_indic]


def load_nco_rl_coeff(data, other_data, ht, is_ref=True, typo="ST"):
    global coeffs_lcr, coeffs_lcr_spec, bc_outflow_lcr
    ...
    param_lcr = coeffs_lcr if is_ref else scm.coeffs_lcr
    param_lcr_spec = coeffs_lcr_spec if is_ref else scm.coeffs_lcr_spec
...
def calculate_lcr(main_data_init, other_data_init=[], dic_indic={}, typo=""):
    ...
    if delta_em_indic in indic_sortie or delta_rl_indic in indic_sortie or delta_nco_indic in indic_sortie \
        ...
        nco_coeff, rl_coeff = load_nco_rl_coeff(main_data, other_data, ht, is_ref=False, typo=typo)
        ...
        if nco_indic in indic_sortie or delta_nco_indic in indic_sortie:
            dic_indic[delta_nco_indic] = calc_delta_nco_indic(dic_indic, outfl_indic, em, delta_em, nco_coeff, \
                                                              hors_bilan, filter_outflow, typo, ht)

        if nco_indic in indic_sortie or rl_indic in indic_sortie:
            if typo == "ST":
                calculate_nco_and_rl(dic_indic, coeff_bilan, indic_sortie, em_indic, nco_indic, rl_indic, \
...
def calc_delta_nco_indic(dic_indic, outfl_indic, em, delta_em, nco_coeff, hors_bilan, filter_outflow, typo, ht):
