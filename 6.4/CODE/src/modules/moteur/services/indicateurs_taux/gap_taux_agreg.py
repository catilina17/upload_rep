...
def calcul_gap_tx_agreg(data, dic_stock_scr, dic_sci, typo, simul, data_dic=[], type_pn=""):
    dic_gp_f = {}
    dic_gp_m = {}
    dic_gp_inf_f = {}
    dic_gp_inf_m = {}
    ...
    if typo == "STOCK":
        lef, lem, tef, tem = get_encours(dic_sci)
        tab_fix_ef_st, tab_fix_em_st = get_tab_fix_st(dic_stock_scr, "STOCK")
    else:
        lef, lem = data_dic["mtx_ef"], data_dic["mtx_em"]
        tef, tem = [], []
        lef_fix_pn, lem_fix_pn = get_data_mtx_fix(data_dic, simul)
        if ((up.force_gps_nmd and simul == "LIQ") or (mp.force_gps_nmd_eve and "EVE" in simul)) and "nmd" in type_pn:
            tab_fix_ef_st, tab_fix_em_st = get_tab_fix_st(dic_stock_scr, "PN", data=data)

    filtre_tla = get_filtres_tla(data, typo)

    conv_ecoul, is_there_conv = get_conv_ecoulement(data, typo, simul)

    coeff_gp_inf, filtre_inf = get_coeff_gp_inf(data)

    coeff_index_ntf = get_coeff_gp_tf(data)

    for k in range(0, up.nb_mois_proj_out + 1):

        calc_gap_liq(k, lef, lem, dic_gp_liq_f, dic_gp_liq_m, conv_ecoul, is_there_conv, typo, simul)

        if typo == "STOCK":
            calculate_gp_tx_refix_st(k, dic_gp_liq_f, dic_gp_liq_m, tef, tem, tab_fix_ef_st, tab_fix_em_st, \
                                     filtre_tla, dic_gp_f, dic_gp_m, is_there_conv, "STOCK", simul)
        else:
            calcul_gap_tx_mtx_pn(k, dic_gp_f, dic_gp_m, lef_fix_pn, lem_fix_pn, simul)
            if "ech" not in type_pn or ("ech" in type_pn and not (up.type_simul["EVE"] or up.type_simul["EVE_LIQ"])):
                clean_mtx_gap(data_dic, k, simul)

            if ((up.force_gps_nmd and simul == "LIQ") or (
                    mp.force_gps_nmd_eve and "EVE" in simul)) and "nmd" in type_pn:
                dic_gp_f_conv = {};
                dic_gp_m_conv = {};
                calculate_gp_tx_refix_st(k, dic_gp_liq_f, dic_gp_liq_m, tef, tem, tab_fix_ef_st, tab_fix_em_st, \
                                         filtre_tla, dic_gp_f_conv, dic_gp_m_conv, is_there_conv, "PN", simul)
                choose_gp_tx_pn(k, dic_gp_f, dic_gp_m, dic_gp_f_conv, dic_gp_m_conv, is_there_conv, simul)

        calculate_gap_inflation(k, dic_gp_liq_f, dic_gp_liq_m, dic_gp_inf_f, dic_gp_inf_m, dic_gp_f, dic_gp_m,
                                coeff_gp_inf, filtre_inf, typo, simul)

        adjust_gp_tf_tla(k, dic_gp_liq_f, dic_gp_liq_m, dic_gp_f, dic_gp_m, coeff_index_ntf, typo, simul)

        calculate_gap_reg(k, dic_gp_rg_f, dic_gp_rg_m, dic_gp_f, dic_gp_inf_f, dic_gp_m, dic_gp_inf_m, typo, simul)

        format_gp_tx(k, data, dic_sci, dic_gp_f, dic_gp_m, dic_gp_inf_f, dic_gp_inf_m, dic_gp_rg_f, dic_gp_rg_m, typo,
                     simul)

    return is_there_conv

def get_data_mtx_fix(data_dic, simul):
    return data_dic["mtx_gp_fix_f_" + simul], data_dic["mtx_gp_fix_m_" + simul]


def clean_mtx_gap(data_dic, k, simul):
...
def calculate_gp_tx_refix_st(k, dic_gp_liq_f, dic_gp_liq_m, tef, tem, tab_fix_f, \
    ...
    if do_f or do_m:
        """ MOIS REFIXING TLA"""
        mois_refix_tla = k + up.freq_refix_tla + 1 if k >= up.mois_refix_tla else up.mois_refix_tla + 1
        ...
        if k >= 1:
            ...
            if do_f:
                ...
                """ PREP REFIXING"""
                tab_fix_f_rol = roll_tab(tab_fix_f, k, typo)
                """ TRAITEMENT TLA """
                tab_fix_f_rol = change_fixing_tla(tab_fix_f_rol, k, filtre_tla, mois_refix_tla)
                """ TRAITEMENT CONV """
            ...
            if do_m:
                ...
                """ PREP REFIXING"""
                tab_fix_m_rol = roll_tab(tab_fix_m, k, typo)
                """ TRAITEMENT TLA """
                tab_fix_m_rol = change_fixing_tla(tab_fix_m_rol, k, filtre_tla, mois_refix_tla)
                """ TRAITEMENT CONV """
...
def calc_gap_liq(k, lef, lem, dic_gp_liq_f, dic_gp_liq_m, conv_ecoul, is_there_conv, typo, simul):
    if "EVE" in simul:
        do_f = False
...
def get_tab_fix_st(dic_stock_scr, typo, data=[]):
    if typo == "PN":
        ht = data.shape[0]
...
def calculate_gap_inflation(k, dic_gp_liq_f, dic_gp_liq_m, dic_gp_inf_f, dic_gp_inf_m, dic_gp_f, dic_gp_m, coeff_gp_inf,
...
def adjust_gp_tf_tla(k, dic_gp_liq_f, dic_gp_liq_m, dic_gp_f, dic_gp_m, coeff_index_ntf, typo, simul):
    if "EVE" == simul:
        do_f = False
        do_m = (di.gap_tx_eve_em_st[k] or k == 0) if typo == "STOCK" else di.gap_tx_eve_em_pn[k]
    elif "EVE_LIQ" == simul:
...
def calculate_gap_reg(k, dic_gp_rg_f, dic_gp_rg_m, dic_gp_f, dic_gp_inf_f, dic_gp_m, dic_gp_inf_m, typo, simul):
    if "EVE" == simul:
        do_f = False
    ...
    if do_f:
        dic_gp_rg_f[k] = up.coeff_reg_tf_usr * dic_gp_f[k] + up.coeff_reg_inf_usr * dic_gp_inf_f[k]
        if k == 0 and typo == "STOCK" and simul == "EVE":
            dic_gp_rg_f["0_NC"] = up.coeff_reg_tf_usr * dic_gp_f["0_NC"] + up.coeff_reg_inf_usr * dic_gp_inf_f["0_NC"]

    if do_m:
        dic_gp_rg_m[k] = up.coeff_reg_tf_usr * dic_gp_m[k] + up.coeff_reg_inf_usr * dic_gp_inf_m[k]
        if k == 0 and typo == "STOCK" and simul == "EVE":
            dic_gp_rg_m["0_NC"] = up.coeff_reg_tf_usr * dic_gp_m["0_NC"] + up.coeff_reg_inf_usr * dic_gp_inf_m["0_NC"]


def get_coeff_gp_inf(data):
    ht = data.shape[0]
    index_calc = np.array(data[gp.nc_output_index_calc_cle].values)
    ...
    choices_coeff_gp_inf = [up.coeff_inf_cel_usr, up.coeff_inf_tla_usr, up.coeff_inf_tlb_usr, 0]

    return np.select(filtres_index, choices_coeff_gp_inf), (np.isin(index_calc, mp.inf_indexes)).reshape(ht, 1)


def get_coeff_gp_tf(data):
    ht = data.shape[0]
    index_calc = np.array(data[gp.nc_output_index_calc_cle].values)
    filtre_cel = (np.isin(index_calc, mp.cel_indexes))
    filtre_tla_u = (np.isin(index_calc, mp.tla_indexes) | np.isin(index_calc, mp.lep_indexes))
    ...
    choices_coeff_gp_tf = [up.coeff_tf_cel_usr, up.coeff_tf_tla_usr, up.coeff_tf_tlb_usr, 0]

    return np.select(filtres_index, choices_coeff_gp_tf)


def get_filtres_tla(data, typo):
    ht = data.shape[0]
    ...
    if typo == "STOCK":
        ...
        filtre_tla = (np.array(is_ech_stock) == False) & (
            np.array(data[gp.nc_output_index_calc_cle].isin(mp.all_gap_gestion_index))) & np.array([up.retraitement_tla])

    else:
        filtre_tla = (np.array(data[gp.nc_output_index_calc_cle].isin(mp.all_gap_gestion_index))) \
                     & np.array([up.retraitement_tla])

    return np.array(filtre_tla).reshape(ht, 1)


def get_conv_ecoulement(data, typo, simul):
    ht = data.shape[0]
    ...
    is_there_conv = ((~np.isnan(conv_ecoul[:, 0])) & \
                     np.array(([
                         (up.force_gps_nmd and simul == "LIQ") or (mp.force_gps_nmd_eve and "EVE" in simul)]))).reshape(
        ht, 1)

    return conv_ecoul, is_there_conv


def format_gp_tx(k, data, dic_sci, dic_gp_f, dic_gp_m, dic_gp_inf_f, dic_gp_inf_m, dic_gp_rg_f, dic_gp_rg_m, typo,
                 simul):
    ...
    for dic_gp, namo in [(dic_gp_f, gp_tx_ef), (dic_gp_m, gp_tx_em), (dic_gp_inf_f, gp_inf_ef),
        ...
        if k in dic_gp:
            ...
            dic_gp[k].columns = ["M" + str(j) for j in range(deb, up.nb_mois_proj_usr + 1)]
            dic_sci[namo + str(k)] = dic_gp[k]
            if k == 0 and typo == "STOCK" and simul == "EVE":
                dic_sci[namo + str(k) + "_NC"] = dic_gp[str(k) + "_NC"]


def calcul_gap_tx_mtx_pn(k, dic_gp_f, dic_gp_m, lef_fix_pn, lem_fix_pn, simul):
...
def choose_gp_tx_pn(k, dic_gp_f, dic_gp_m, dic_gp_f_conv, dic_gp_m_conv, is_there_conv, simul):
    do_f = di.gap_tx_ef_pn[k] if simul == "LIQ" else False
    do_m = di.gap_tx_em_pn[k] if simul == "LIQ" else di.gap_tx_eve_em_pn[k]
...
def change_fixing_tla(tab, k, filtre_tla, mois_refix_tla):
    """ TRAITEMENT TLA """
...
def calcul_gap_tx_ajust(dic_ajust, num_cols, simul):
    ...
    for p in range(0, up.nb_mois_proj_out + 1):
        ...
        if do_f:
            ...
            dic_ajust[gp_reg_f + str(p)][cols] = up.coeff_reg_tf_usr * dic_ajust[
                gp_tx_f + str(p)][cols] + up.coeff_reg_inf_usr * dic_ajust[gp_inf_f + str(p)][cols]

        if do_m:
            cols = [x for x in num_cols if pref + str(p) != x]
            dic_ajust[gp_reg_m + str(p)] = dic_ajust[ind_em].copy()
            dic_ajust[gp_reg_m + str(p)][gp.nc_output_ind3] = gp.gpr_em_pni + str(p)
            dic_ajust[gp_reg_m + str(p)][cols] = up.coeff_reg_tf_usr * dic_ajust[
                gp_inf_m + str(p)][cols] + up.coeff_reg_inf_usr * dic_ajust[gp_tx_m + str(p)][cols]
