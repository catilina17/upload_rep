...
def calcul_eve(dic_sci, main_data, typo, cols=[]):
    """ Module permettant de calculer l'EVE """

    indic_sortie = get_indic_sorties(typo)

    if len(indic_sortie) > 0:
        eve_em, fk_em, fi_em, gap_em, deb, fk_em_act, fi_em_act, gap_em_liq = choose_indicators(typo)

        if typo == "AJUST":
            main_data, deb = format_data_ajust(dic_sci, cols)

        is_ra_rn = get_rarn_immo_perimeter(main_data, typo)
        ...
        if gf.begin_in_list(indic_sortie, eve_em) or gf.begin_in_list(indic_sortie, fk_em_act) \
            ...
            zc_ht = tx.max_month_zc
            if typo == "ST":
                devises_data = main_data[[gp.nc_output_devise_cle]].copy().reset_index(level=[gp.nc_output_contrat_cle])
            elif typo == "PN":
            ...
            devises_data[gp.nc_output_bilan] = np.where(devises_data[gp.nc_output_contrat_cle].str[:3].isin(["AHB", "PHB"]),
                                                        "HB", "B")
            devises_data[gp.nc_output_contrat_cle] \
                = np.where(devises_data[gp.nc_output_contrat_cle].str[:6].isin(["AHB-NS", "PHB-NS"]), "AHB-NS*,PHB-NS*",
                           "*")

            list_devises = mp.act_eve.index.get_level_values("DEVISE").values
            default = ~devises_data[gp.nc_output_devise_cle].isin(list_devises)
            devises_data.loc[default, gp.nc_output_devise_cle] = "*"

            zc_taux = get_act_curve(devises_data, ht, zc_ht)
        ...
        for p in range(deb, up.nb_mois_proj_out + 1):
            ...
            if eve_em + str(p) in indic_sortie or fk_em + str(p) in indic_sortie:
                calc_flux_cap(dic_sci, main_data, fk_em, gap_em, gap_em_liq, is_ra_rn, p, typo, M)

            """ FLUX INTERET"""
            if eve_em + str(p) in indic_sortie or fi_em + str(p) in indic_sortie:
                mni_p = format_mni(dic_sci, typo, p)

            if eve_em + str(p) in indic_sortie or fi_em + str(p) in indic_sortie:
                calc_flux_int(dic_sci, gap_em, fi_em, main_data, p, typo, mni_p, M)

            """ EVE"""
            if eve_em + str(p) in indic_sortie or fk_em_act + str(p) in indic_sortie \
                    or fi_em_act + str(p) in indic_sortie:
                """ ACTUALISATION """
                tx_act = calc_tx_actu(zc_taux, zc_ht, ht, p, typo)

            if eve_em + str(p) in indic_sortie:
                calc_eve(dic_sci, main_data, eve_em, fi_em, fk_em, tx_act, p, M, typo, gap_em)

            if fk_em_act + str(p) in indic_sortie:
                calc_flux_cap_act(dic_sci, fk_em_act, fk_em, tx_act, p)

            if fi_em_act + str(p) in indic_sortie:
                calc_flux_int_act(dic_sci, fi_em_act, fi_em, tx_act, p)

        if typo == "AJUST":
            concat_eve_with_ajust_data(dic_sci, main_data)


def get_rarn_immo_perimeter(main_data, typo):
    if typo == "ST":
        n = main_data.shape[0]
    ...
    is_ra_rn = is_ra_rn & np.full(is_ra_rn.shape[0], mp.mode_cal_gap_tx_immo != "GAP TAUX")

    return np.array(is_ra_rn).reshape(main_data.shape[0], 1)


def make_zero_for_specific_contracts(main_data, dic_ind, indic, typo):
    contracts_to_exclude = mp.eve_contracts_excl.index.tolist()
    if "CAP" in indic:
...
def format_data_ajust(dic_sci, cols):
...
def get_indic_sorties(typo):
    if typo == "ST":
        indic_sortie = up.indic_sortie_eve["ST"]
    else:
        indic_sortie = up.indic_sortie_eve["PN"] if typo == "PN" else up.indic_sortie_eve["AJUST"]

    return indic_sortie

def choose_indicators(typo):
    if typo == "ST":
...
def concat_eve_with_ajust_data(dic_sci, main_data):
    for indic in dic_sci:
...
def calc_eve(dic_sci, main_data, eve_ind, fi_ind, fk_ind, tx_act, p, M, typo, gap_ind):
    ...
    make_zero_for_specific_contracts(main_data, dic_sci, eve_ind + str(p), typo)


def calc_flux_cap_act(dic_sci, fk_act_ind, fk_ind, tx_act, p):
    dic_sci[fk_act_ind + str(p)] = tx_act * (dic_sci[fk_ind + str(p)])


def calc_flux_int_act(dic_sci, fi_act_ind, fi_ind, tx_act, p):
    dic_sci[fi_act_ind + str(p)] = tx_act * (dic_sci[fi_ind + str(p)])


def calc_tx_actu(zc_taux, zc_ht, ht, p, typo):
    delta_days = tx.delta_days.copy()[:up.nb_mois_proj_usr + 1].reshape(1, up.nb_mois_proj_usr + 1, zc_ht)
    taux_floor_reg = load_tax_reg()

    tx_act = (1 / ((1 + np.maximum(zc_taux[:, :, p], taux_floor_reg)) ** (
            delta_days[:, :, p] / 365))).reshape(ht, up.nb_mois_proj_usr + 1)
    if typo == "PN":
        tx_act = tx_act[:, 1:]
...
def format_mni(dic_sci, typo, p):
    if typo in ["PN", "AJUST"]:
        ...
        if up.type_eve == "ICAAP":
            mni_p = dic_sci[gp.mn_liq_eve_sti][cols]
...
def load_tax_reg():
    taux_floor_reg = np.column_stack([np.array([mp.eve_reg_floor[0, 0]]).reshape(1, 1), \
                                      np.array(mp.eve_reg_floor)])
    taux_floor_reg = taux_floor_reg[:, :up.nb_mois_proj_usr + 1].reshape(1, up.nb_mois_proj_usr + 1)
    return taux_floor_reg


def get_act_curve(devises_data, ht, zc_ht):
    devises_data = devises_data.set_index([gp.nc_output_devise_cle, gp.nc_output_bilan,
                                           gp.nc_output_contrat_cle])
    pricing_curve_inner = devises_data.join(mp.act_eve, how="inner")[[gp.nc_act_eve_act_curve]].copy()
    ...
    pricing_curve = devises_data.join(mp.act_eve, how="left",
                                      on=[gp.nc_output_devise_cle, gp.nc_output_bilan,
                                          gp.nc_output_contrat_cle])[[gp.nc_act_eve_act_curve]].copy()

    zc_curve_all = tx.zc_all_curves.copy()
    absent_curve = \
    pricing_curve[~pricing_curve[gp.nc_act_eve_act_curve].isin(zc_curve_all[gp.nc_pricing_curve].values.tolist())][
        gp.nc_act_eve_act_curve]
    if len(absent_curve):
    ...
    zc_curve = \
    pricing_curve.merge(zc_curve_all, how="left", right_on=[gp.nc_pricing_curve], left_on=[gp.nc_act_eve_act_curve])[
        [x for x in zc_curve_all if x != gp.nc_pricing_curve]].copy()
    zc_curve = np.array(zc_curve).reshape(ht, int(zc_curve.shape[0] / ht), zc_ht)[:, :up.nb_mois_proj_usr + 1, :]

    return zc_curve


def calc_flux_int(dic_sci, gap_ind, fi_ind, main_data, p, typo, mni_p, M):
    ...
    make_zero_for_specific_contracts(main_data, dic_sci, fi_ind + str(p), typo)


def calc_flux_cap(dic_sci, main_data, flux_cap, gap_ind, gap_em_liq, is_ra_rn, p, typo, M):
    ...
    dic_sci[flux_cap + str(p)]["M" + str(up.nb_mois_proj_usr)] = gap_rg["M" + str(up.nb_mois_proj_usr - 1)]

    make_zero_for_specific_contracts(main_data, dic_sci, flux_cap + str(p), typo)
