...
import modules.moteur.services.indicateurs_taux.ech.ech_mtx_gap_tf as gp_ech
...
class PN_ECH_PROJECTER():
    ...
    def __init__(self, cls_calc, cls_pn_loader, scen_path, scenario_name, ech_prct=""):
        self.IND = "ech"
    ...
    def project_pn_ech(self, data_ech, dic_pn_sc, dic_stock_sci="", ):
        if len(self.type_scenario) > 0 and data_ech.shape[0] > 0 and not (
                ...
                and up.indic_sortie["AJUST"] + up.indic_sortie_eve["AJUST"] == []) and self.max_month_pn > 0:
            self.ht = data_ech.shape[0] // 4
            self.lg = up.nb_mois_proj_usr
    ...
    def get_profiles(self, compiled_indics, key_vars_dic):
        ...
        cols_num = ["M%s" % i for i in range(1, up.nb_mois_proj_usr + 1)]
        dic_profiles = {}
    ...
    def calc_tx_cli(self, dic_pn_sc, dic_profiles):
        ...
        tx_cli = np.concatenate(
            ...
            [np.zeros((self.ht, up.nb_mois_proj_usr - self.max_month_pn))], axis=1)
    ...
    def load_coeff_mens(self, data_pn):
        base_calc = hf.calendar_coeff.copy()[
        ...
        d_coeff.columns = ["M" + str(i) for i in range(1, up.nb_mois_proj_usr + 1)]
