...
global data_adj_mni, data_adj_eve
data_adj_mni = []
data_adj_eve =  []
...
def prepare_adjustements(data, type="NORMAL"):
    global data_adj_mni, data_adj_eve
    ...
    list_fx_swaps = mp.contrats_fx_swaps.index.values.tolist()
    ...
    if type != "EVE":
        if len(data_adj_mni) == 0:
            data_adj_mni = data.reset_index(drop=True)
        else:
            data_adj_mni = pd.concat([data_adj_mni, data.reset_index(drop=True)])
    else:
        if len(data_adj_eve) == 0:
            data_adj_eve = data.reset_index(drop=True)
        else:
            data_adj_eve = pd.concat([data_adj_eve, data.reset_index(drop=True)])


def add_adjustments(dic_ajust, type="NORMAL"):
    global data_adj_mni, data_adj_eve
    data_adj = data_adj_mni.copy() if type != "EVE" else data_adj_eve.copy()
    ind_em = gp.em_sti if type != "EVE" else gp.em_eve_sti
    ind_ef = gp.ef_sti if type != "EVE" else gp.ef_eve_sti
    if (len(up.indic_sortie["AJUST"]) > 0 or len(up.indic_sortie_eve["AJUST"]) > 0) and len(data_adj) > 0:
        ...
        data_devise[gp.nc_output_devise_cle] = np.where(~data_devise.iloc[:, 0].isin(up.courbes_ajust.index.values.tolist()),
                                                        "*", data_devise.iloc[:, 0].values)
        data_adj[["CURVE_NAME","TENOR"]]\
            = data_devise.join(up.courbes_ajust,on=[gp.nc_output_devise_cle]).iloc[:,-2:]
        ...
        for p in range(1, up.nb_mois_proj_out + 1):
            if ind_ef + str(p) in up.indic_sortie["AJUST"]:
            ...
            if ind_em + str(p) in up.indic_sortie["AJUST"]:
...
def divide_by_actif_passif(dic_ajust, data_adj_all, num_cols, type="NORMAL"):
    ind_em = gp.em_sti if type != "EVE" else gp.em_eve_sti
...
def calculate_indics(dic_ajust, name_scenario, save_path):
    if dic_ajust !={}:

        indic_sortie = up.indic_sortie["AJUST"] + up.indic_sortie_eve["AJUST"]
        num_cols = ["M0" + str(k) if k < 10 else "M" + str(k) for k in range(0, up.nb_mois_proj_usr + 1)]

        """ GAP TX """
        for simul in list(up.type_simul.keys()):
            if up.type_simul[simul] == True:
                gp_tx.calcul_gap_tx_ajust(dic_ajust, num_cols, simul)
        ...
        data_adj_all_eve = data_adj_all[data_adj_all[gp.nc_output_ind3].isin(up.indic_sortie_eve["AJUST"] + [gp.em_eve_pni])]
        data_adj_all_liq = data_adj_all[data_adj_all[gp.nc_output_ind3].isin(up.indic_sortie["AJUST"] + [gp.em_pni])]

        dic_ajust_eve = {key: val for key, val in dic_ajust.items() if key in up.indic_sortie_eve["AJUST"] + [gp.em_eve_pni]}
        dic_ajust_liq = {key: val for key, val in dic_ajust.items() if key in up.indic_sortie["AJUST"] + [gp.em_pni]}

        data_compil_adjs = divide_by_actif_passif(dic_ajust_liq, data_adj_all_liq, num_cols)

        if len(dic_ajust_eve) > 0:
            data_compil_adjs_eve = divide_by_actif_passif(dic_ajust_eve, data_adj_all_eve, num_cols, type="EVE")
        ...
        """ MAPPING LIQ"""
        data_compil_adjs = mp.map_liq(data_compil_adjs, override=True)
        ...
        data_compil_adjs = data_compil_adjs[[x for x in data_compil_adjs.columns if x not in up.cols_sortie_excl]]
        ...
        data_compil_adjs = data_compil_adjs.drop(["M0" + str(k) if k < 10 else "M" + str(k) for k in
                                                  range(up.nb_mois_proj_out + 1, up.nb_mois_proj_usr + 1)], axis=1)
        ...
        name_file = gp.nom_compil_st_pn if up.merge_compil else gp.nom_compil_pn
        ...
        rw_ut.numpy_write(data_compil_adjs, save_path + "\\" + name_file, up.nb_mois_proj_out,  encoding='cp1252', mode=mode, header=header, \
