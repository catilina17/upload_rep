...
class Agregate_to_PASSALM_Level():
    ...
    def join_with_stock_data(self, results, stock_data):
        ...
        results_joined = results.join(stock_data, on=["TEMP_KEY"])

        results_joined = self.filter_zero_sum_products(results_joined)
    ...
    def filter_zero_sum_products(self, results_joined):
        zero_prod = (results_joined[self.cols_month].values.sum(axis=1) == 0) & (results_joined[pa.NC_PA_IND03] == "B$LEM")
        index_zero = results_joined.loc[zero_prod, pa.NC_PA_CLE_OUTPUT].apply(lambda row: '_'.join(row.values.astype(str)),
                                                                       axis=1).unique().tolist()
        is_index_zero = results_joined[pa.NC_PA_CLE_OUTPUT].apply(lambda row: '_'.join(row.values.astype(str)),
                                                           axis=1).isin(index_zero)
        results_filtred = results_joined[~(is_index_zero & (~results_joined[pa.NC_PA_CONTRACT_TYPE].str.contains("DAV")))]
    ...
    def mapp_results(self, results):
        ...
        elif "security" in self.product:
            is_action = (results[self.key_vars_dic["CONTRACT_TYPE"]] == "A-ACTION").values
            is_nan = results[self.key_vars_dic["RATE CODE"]].isnull().values
            results[self.key_vars_dic["RATE CODE"]]\
                = np.select([is_action & is_nan,  (~is_action) & is_nan], ["TVZERO", "FIXE"],
                            default=results[self.key_vars_dic["RATE CODE"]].values)
        else:
            results[self.key_vars_dic["RATE CODE"]] = results[self.key_vars_dic["RATE CODE"]].fillna("FIXE").replace("", "FIXE")
        ...
        results = map.map_data_with_general_mappings(results, map.map_pass_alm, self.key_vars_dic)
