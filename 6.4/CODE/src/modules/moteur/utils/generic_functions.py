...
def prolong_last_col_value(data_all, nb_col_sup, s = 0, is_df=False, month_gr=False, year_gr=False, month_diff=False, year_diff=False,\
...
def unfilter_all_sheets(wb):
    for wsh in gp.list_tabs_stock + gp.list_tabs_sc_tx + gp.list_tabs_sc_vol + gp.list_tabs_sc_lcr_nsfr:
        try:
            wb.Worksheets(wsh).Unprotect(Password=gp.password)
            wb.Worksheets(wsh).ShowAllData()
            wb.Worksheets(wsh).Protect(Password=gp.password)
        except:
            pass
