...
def map_and_calculate_duree(data_ech):
    ...
    if up.current_etab in gp.MTY_DETAILED_ETAB:
...
def join_fermat_margins(dic_evol, data_ech, data_nmd):
    new_margins = None
    new_sp_tx = None
...
def get_relevant_stock_data(data_stock):
    ...
    for type_pn in ["NMD", "ECH"]:
        if len(dic_stock_evol[type_pn]) == 0:
            dic_stock_evol[type_pn] = []

    return dic_stock_evol


def add_missing_indics(pn_data, type_pn):
    DEM = pa.NC_PA_DEM_CIBLE
    if type_pn == "ECH":
...
def calculate_PN_PRCT(data_stock, data_ech=[], data_nmd=[]):
    dic_stock_evol = get_relevant_stock_data(data_stock)
    num_cols = [x for x in pa.NC_PA_COL_SORTIE_NUM_ST if x != "M0"]
    if len(dic_stock_evol["ECH"]) > 0:
        dic_stock_evol["ECH"] = map_and_calculate_duree(dic_stock_evol["ECH"])

    if up.current_etab not in gp.NON_RZO_ETABS:
        join_fermat_margins(dic_stock_evol, data_ech.copy(), data_nmd.copy())

    for symb, type_pn in zip(["ECH%", "NMD%"], ["ECH", "NMD"]):
        if len(dic_stock_evol[type_pn]) > 0:
            DEM = pa.NC_PA_DEM_CIBLE
            if ((type_pn == "ECH" and pa.NC_PA_MG_CO in dic_stock_evol[type_pn][pa.NC_PA_IND03].unique()) \
                or (type_pn == "NMD" and up.current_etab not in gp.NON_RZO_ETABS and pa.NC_PA_TX_SP in
            ...
            dic_stock_evol[type_pn] = add_missing_indics(dic_stock_evol[type_pn], type_pn)
