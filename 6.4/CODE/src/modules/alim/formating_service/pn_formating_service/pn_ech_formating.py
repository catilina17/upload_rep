...
def read_marge_file():
    ...
    pn_marge.loc[:, cols_ECH] = pn_marge.loc[:, cols_ECH].fillna(0).astype(np.float64)
    pn_marge.rename(columns={NC_ECH_ETAB2: NC_ECH_ETAB}, inplace=True)
    pn_marge = pn_marge[[NC_ECH_ETAB, NC_ECH_CLE] + cols_ECH].copy()

    pn_marge.columns = [str(col) + '_MARGE' if col in cols_ECH else str(col) for col in pn_marge.columns]

    return pn_marge


def read_encours_file():
    ...
    filter_non_zero = (pn_encours[cols_ECH].sum(axis=1) != 0)
    pn_encours = pn_encours[filter_non_zero].copy()

    if sum(filter_non_zero) > 0:
        ht = pn_encours.shape[0]
        pn_encours[cols_ECH] = pn_encours[cols_ECH].fillna(0).astype(np.float64)
        pn_encours['COEF'] = 0
        filtre1 = (pn_encours[NC_ECH_CONTRACT_TYPE].str[0:2] == "A-") | (
                    pn_encours[NC_ECH_CONTRACT_TYPE].str[-2:] == "-A") & (
                              pn_encours[NC_ECH_CONTRACT_TYPE].str[:2] == "HB")
        filtre2 = (pn_encours[NC_ECH_CONTRACT_TYPE].str[0:2] == "P-") | (
                    pn_encours[NC_ECH_CONTRACT_TYPE].str[-2:] == "-P") & (
                              pn_encours[NC_ECH_CONTRACT_TYPE].str[:2] == "HB")
        pn_encours[cols_ECH] = np.select([np.array(filtre1).reshape(ht, 1), np.array(filtre2).reshape(ht, 1)], \
                                         [pn_encours[cols_ECH], -1 * pn_encours[cols_ECH]], 0)
...
def rename_num_cols(pn_ech_data):
...
def join_marge_and_encours(pn_encours, pn_marge):
    pn_encours_joined = pn_encours.merge(pn_marge, how="left", on=[NC_ECH_CLE, NC_ECH_ETAB])
    ...
    pn_encours_joined[cols_marge] = pn_encours_joined[cols_marge].fillna(0).astype(np.float64)

    return pn_encours_joined


def format_bilan(data):
    filtres = [data[NC_ECH_CONTRACT_TYPE].str[:2] == "A-",
               data[NC_ECH_CONTRACT_TYPE].str[:2] == "P-", \
               (data[NC_ECH_CONTRACT_TYPE].str[-2:] == "-A") & (data[NC_ECH_CONTRACT_TYPE].str[:2] == "HB"),
               (data[NC_ECH_CONTRACT_TYPE].str[-2:] == "-P") & (data[NC_ECH_CONTRACT_TYPE].str[:2] == "HB")]
...
def map_ech_data(pn_ech_data):
    pn_ech_data[NC_ECH_INDEX_REF] = np.where(pn_ech_data[NC_ECH_RATE_CAT] == "FIXED", pn_ech_data[NC_ECH_RATE_CAT],
                                             pn_ech_data[NC_ECH_INDEX_REF])

    pn_ech_data[NC_ECH_CONTREPARTIE] = pn_ech_data[NC_ECH_CONTREPARTIE].fillna("-")

    pn_ech_data = gu.force_integer_to_string(pn_ech_data, NC_ECH_CONTREPARTIE)

    pn_ech_data[NC_ECH_FAMILY] = pn_ech_data[NC_ECH_FAMILY].fillna("-")

    if vp.version_sources == "csv":
        pn_ech_data[NC_ECH_DEBUT] = pn_ech_data[NC_ECH_DEBUT].astype(np.float64)
    ...
    pn_ech_data[[NC_ECH_PERIOD_INTERETS, NC_ECH_PERIOD_CAPI, NC_ECH_PERIOD_AMOR]] \
        = pn_ech_data[[NC_ECH_PERIOD_INTERETS, NC_ECH_PERIOD_CAPI, NC_ECH_PERIOD_AMOR]].fillna("None")

    cles_data = {1: [pa.NC_PA_BILAN, NC_ECH_CONTRACT_TYPE],
                 2: [NC_ECH_INDEX_REF], 3: [NC_ECH_CONTREPARTIE], 4: [NC_ECH_PROFIL_EC],
                 5: [NC_ECH_PERIOD_INTERETS], \
                 6: [NC_ECH_PERIOD_AMOR], 7: [NC_ECH_PERIOD_CAPI]}
    ...
    if vp.version_sources == "csv":
        pn_ech_data[pa.NC_PA_MATURITY_DURATION] = pn_ech_data[NC_ECH_DUREE].fillna("0").str.replace("M", "").astype(np.int64)
    else:
        pn_ech_data[pa.NC_PA_MATURITY_DURATION] = pn_ech_data[NC_ECH_DUREE].fillna("0").str.replace("M", "").astype(np.int64)

    pn_ech_data[pa.NC_PA_MATUR] = np.where(pn_ech_data[pa.NC_PA_MATURITY_DURATION] < 13, "CT", "MLT")

    pn_ech_data.rename(columns={NC_ECH_FAMILY: pa.NC_PA_MARCHE, NC_ECH_BASE_CAL: pa.NC_PA_ACCRUAL_BASIS,
                                NC_ECH_INDEX_REF: pa.NC_PA_RATE_CODE, NC_ECH_CCY_CODE: pa.NC_PA_DEVISE}, inplace=True)

    pn_ech_data[pa.NC_PA_ACCRUAL_BASIS] = pn_ech_data[pa.NC_PA_ACCRUAL_BASIS].str.upper()

    pn_ech_data[NC_ECH_RELEASING_RULE] = pn_ech_data[NC_ECH_RELEASING_RULE].fillna(-1).astype(int).astype(str).replace("-1", "")

    pn_ech_data = pn_ech_data.rename(columns={NC_ECH_PERIOD_FIXING: pa.NC_PA_FIXING_PERIODICITY,
                                              NC_ECH_DEBUT: pa.NC_PA_JR_PN, NC_ECH_RELEASING_RULE: pa.NC_PA_RELEASING_RULE})

    pn_ech_data[pa.NC_PA_FIXING_PERIODICITY] = pn_ech_data[pa.NC_PA_FIXING_PERIODICITY].fillna("")

    col_sortie = [x for x in pa.NC_PA_COL_SORTIE_QUAL_ECH if x != NC_ECH_CLE]
    keep_cols = [x for x in col_sortie if x in pn_ech_data.columns] + cols_ECH + cols_marge
    pn_ech_data = pn_ech_data[keep_cols].copy()

    return pn_ech_data


def add_map_cols(pn_encours):
...
def map_lcr_nsfr_coeff(pn_encours):
...
def add_contracts_mapp_cols(pn_data, type_pn):
    pn_data = mp.map_data(pn_data, gmp.map_pass_alm["CONTRATS"], \
                          keys_data=[pa.NC_PA_BILAN, NC_ECH_CONTRACT_TYPE],
...
def add_missing_cols(pn_ech_data):
...
def group_ech_data(pn_ech_data):
    all_cols_except_dem = [x for x in pn_ech_data.columns if
                           x not in cols_ECH]  # marges incluses dans les colonnes d'agreg
...
def add_missing_indics(pn_ech_data):
...
def final_formatting(pn_ech_data):
    pn_ech_data = add_missing_num_cols(pn_ech_data)
    pn_ech_data = add_missing_indics(pn_ech_data)
    return pn_ech_data[pa.NC_PA_COL_SORTIE_QUAL_ECH + pa.NC_PA_COL_SORTIE_NUM_PN]


def change_to_to_row_format(pn_ech_data):
    pn_ech_data = gu.unpivot_data(pn_ech_data, pa.NC_PA_IND03)
    return pn_ech_data


def format_RZO_PN_ECH():
    logger.info("TRAITEMENT DES PN ECH")
    pn_marge = read_marge_file()
    pn_encours = read_encours_file()

    if len(pn_encours) > 0:

        pn_encours = format_bilan(pn_encours)

        pn_ech_data = join_marge_and_encours(pn_encours, pn_marge)

        pn_ech_data = map_ech_data(pn_ech_data)

        pn_ech_data = group_ech_data(pn_ech_data)

        pn_ech_data = add_map_cols(pn_ech_data)

        pn_ech_data = rename_num_cols(pn_ech_data)

        pn_ech_data = map_lcr_nsfr_coeff(pn_ech_data)

        pn_ech_data = mp.mapping_consolidation_liquidite(pn_ech_data)

        pn_ech_data = add_missing_cols(pn_ech_data)

        pn_ech_data = change_to_to_row_format(pn_ech_data)

        pn_ech_data = final_formatting(pn_ech_data)
