...
def add_missing_num_cols(data, num_cols, prefix, necessary_cols, force_zero=False):
    missing_cols = [x for x in necessary_cols if x not in num_cols]
    warning_cols = []
    for x in missing_cols:
        if not force_zero:
            if ("SWAP" in prefix or "LMN" in prefix or "LMN EVE" in prefix) and x > (up.dar + relativedelta(months=60)).date():
...
def select_num_cols(data, prefix, force_zero=False):
    ...
    necessary_cols = [(up.dar + relativedelta(months=x) + relativedelta(day=31)).date() for x in pa.ALL_NUM_STOCK]

    data = add_missing_num_cols(data, num_cols, prefix, necessary_cols, force_zero=force_zero)
    ...
    if prefix in ["SWAP","LMN", "LMN EVE"] and up.current_etab in gp.NTX_FORMAT:
