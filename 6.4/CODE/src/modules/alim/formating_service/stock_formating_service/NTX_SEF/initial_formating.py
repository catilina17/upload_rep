...
def filter_data_depart_decale(data):
...
def do_preliminary_checks(data, file_name):
...
def modify_contracts_for_lcr_tiers(data):
    ...
    data[gp.NC_CONTRACT_TEMP] = np.select([checkA, checkP], [ntx_p.contract_change_actif, ntx_p.contract_change_passif], \
                                          default=data[gp.NC_CONTRACT_TEMP])

    return data


def transform_num_columns_names(data):
...
def determine_scope_and_marche(data):
    ...
    if ntx_p.NC_NTX_DATA_TYPE_EMETTEUR in data.columns.tolist():
        data[pa.NC_PA_MARCHE] = np.where((data[ntx_p.NC_NTX_DATA_SWAP_TAG].isin([1, 2])) & (data[ntx_p.NC_NTX_DATA_TYPE_EMETTEUR]!=""),\
                                      data[ntx_p.NC_NTX_DATA_TYPE_EMETTEUR], data[pa.NC_PA_MARCHE])
        data.drop([ntx_p.NC_NTX_DATA_TYPE_EMETTEUR],axis=1, inplace=True)
    return data


def format_id_column(data, prefix):
    ...
    if data[ntx_p.NC_NTX_DATA_ID].duplicated().any():
        ...
        logger.error("Non unique ids are: %s" % data[ntx_p.NC_NTX_DATA_ID][data[ntx_p.NC_NTX_DATA_ID].duplicated()].values.tolist())
...
def add_missing_vars(data):
    data[pa.NC_PA_BASSIN] = up.current_etab
    for col in [pa.NC_PA_ETAB, pa.NC_PA_CUST, pa.NC_PA_PERIMETRE, pa.NC_PA_SOUS_METIER, pa.NC_PA_ZONE_GEO, pa.NC_PA_METIER, pa.NC_PA_SOUS_ZONE_GEO]:
...
def finalize_formatting(data):
    ...
    data = filter_data_depart_decale(data)
    data = filter_encours_zero(data)
...
def format_hors_bilan(data, num_cols):
...
def read_and_format_ntx_file(file_name, prefix):
    ...
    data = transform_num_columns_names(data)
    data = do_preliminary_checks(data, file_name)
    data = format_id_column(data, prefix)
    data, num_cols = init_f_gen.select_num_cols(data, prefix)

    if prefix == "LEF":
        data = rename_columns(data, prefix)
        data = init_f_gen.select_cols_to_keep(data, ntx_p.cols_to_keep, num_cols)
        data = init_f_gen.format_bilan_column(data)
        data = modify_contracts_for_lcr_tiers(data)
        data = init_f_gen.format_paliers(data, gp.NC_PALIER_TEMP)
        data = format_type_emetteur(data)
        data = init_f_gen.upper_non_num_cols_vals(data, num_cols)
        data = format_hors_bilan(data, num_cols)
    else:
...
def agregate_data(data, agreg_vars):
...
def read_and_join_all_files():
    i = 0
    for file in up.main_files_name:
        if i == 0 and up.prefixes_main_files[i] != "LEF":
            raise ValueError("Le fichier GAP LIQ doit être lu en premier")

        logger.info("   Pour l'indicateur " + up.prefixes_main_files[i] + ", lecture de : " + file.split("\\")[-1])
        data_prefix = read_and_format_ntx_file(file, up.prefixes_main_files[i])
        if i == 0:
            STOCK_DATA = data_prefix
        else:
            STOCK_DATA = init_f_gen.append_data(STOCK_DATA, data_prefix, up.prefixes_main_files[i])

        i = i + 1

    STOCK_DATA = finalize_formatting(STOCK_DATA)

    STOCK_DATA = determine_scope_and_marche(STOCK_DATA)

    STOCK_DATA = agregate_data(STOCK_DATA, ntx_p.agreg_vars)

    STOCK_DATA = add_missing_vars(STOCK_DATA)
