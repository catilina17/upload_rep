...
import modules.alim.formating_service.stock_formating_service.NTX_SEF.initial_formating as ntx_f
import logging
import numpy as np
...
def format_montants_RZO(data, prefix):
...
def do_preliminary_checks(data, file_name):
    try:
        check = datetime.strptime(str(data[os_p.NC_OS_DATA_DAR].iloc[0]), '%d/%m/%Y').date() != up.dar.date()
    except:
        check = datetime.strptime(str(data[os_p.NC_OS_DATA_DAR].iloc[0]), '%Y-%m-%d').date() != up.dar.date()
...
def read_os_format_files(file_name, prefix):
    global data_st_oney
    if up.current_etab == "ONEY":
    ...
    data = format_montants_RZO(data, prefix)
    data, num_cols = init_f_gen.select_num_cols(data, prefix, force_zero=True)
    data = init_f_gen.upper_non_num_cols_vals(data, num_cols)
    if up.current_etab == "SOCFIM":
        data = ntx_f.format_hors_bilan(data, num_cols)
    else:
        data = format_hors_bilan(data)
        if prefix == "LEF":
            data_st_oney = data.copy()
...
def add_missing_vars(data):
    ...
    data[pa.NC_PA_ETAB] = up.current_etab
    ...
    data = gu.add_constant_new_col_pd(data,\
                                      ...
                                      [gp.MNI_AND_LIQ, 100, up.current_etab])
...
def read_and_join_all_files():
    ...
    for file in up.main_files_name:
        ...
        data_prefix = read_os_format_files(file, up.prefixes_main_files[i])
        if i == 0:
            STOCK_DATA = data_prefix.copy()
        else:
            STOCK_DATA = init_f_gen.append_data(STOCK_DATA.copy(), data_prefix.copy(), up.prefixes_main_files[i],
                                                warning=False)
        i = i + 1
