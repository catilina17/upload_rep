...
def do_preliminary_checks(data, file_name):
    if datetime.strptime(str(data[rzo_p.NC_RZO_DATA_DAR].iloc[0]), '%Y%m%d').date() != up.dar.date():
...
def read_rzo_format_files(file_name, prefix):
...
def read_and_join_all_files():
    ...
    for file in up.main_files_name:
        ...
        data_prefix = read_rzo_format_files(file, up.prefixes_main_files[i])
        if i == 0:
            STOCK_DATA = data_prefix.copy()
        else:
            if up.prefixes_main_files[i] not in prefixes_done:
        ...
        prefixes_done.append(up.prefixes_main_files[i])
        i = i + 1
