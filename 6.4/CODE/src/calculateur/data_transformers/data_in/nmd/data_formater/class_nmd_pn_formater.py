...
class Data_NMD_PN_Formater():
    ...
    def create_groups_by_nb_parts_no_neg(self, data_nmd_rm_all):
        data_nmd_rm_all["ICNE"] = np.where(
            data_nmd_rm_all[self.cls_fields.NC_LDP_CONTRACT_TYPE].str.contains("ICNE").values, "ICNE", "NOT_ICNE")
        i = 1
        dic_data_nmd = {}
        dic_data_nmd_tmp = dict(tuple(data_nmd_rm_all.groupby(["NB_PARTS", "HAS_VOLATILE", "ICNE"], dropna=False)))
        cur_len = 0
    ...
    def filter_data(self, data):
        ...
        #data = data[data[self.cls_pa_fields.NC_PA_CONTRACT_TYPE].str.contains("P-PEL")].copy()
        #data = data[data[self.cls_pa_fields.NC_PA_ETAB].isin(["BRED"])].copy()
        return data
    ...
    def generate_templated_pn_data(self):
        ...
        data_nmd_pn_filtered = self.filter_data(data_nmd_pn)
