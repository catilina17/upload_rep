...
def run_calculator_stock_ech(dar, horizon, source_data, name_product, tx_params=[],
                             ...
                             cr_immo_cal_mode="quick", output_mode="dump"):
    try:

        cls_st = load_stock_data(source_data["STOCK"], dar, batch_size, name_product, horizon)
...
def load_stock_data(source_data, dar_usr, batch_size, name_product, horizon):
    cls_fields = Data_Fields()
    cls_format = Data_Formater(cls_fields)
    cls_pa_fields = PASS_ALM_Fields()

    cls_st = Data_ECH_STOCK(cls_fields, cls_format, cls_pa_fields, source_data, dar_usr, batch_size, name_product, horizon)
